...
class Car:
    def __init__(self, comfort_class: str, clean_mark: float, brand: str) -> None:
        self.comfort_class = comfort_class.lower()
        self.clean_mark = float(clean_mark)
        self.brand = brand

    def __repr__(self) -> str:
        return f"Car({self.brand}, {self.comfort_class}, {self.clean_mark})"
...
class CarWashStation:
    ...
    def calculate_washing_price(self, car: Car) -> float:
        ...
        dirtiness_multiplier = 1.0 + dirtiness
        distance_surcharge = 0.5 * self.distance_from_city_center
        power_discount = max(0.0, min(0.20, (self.clean_power - 50.0) / 250.0))
        price = base * dirtiness_multiplier * (1 - power_discount) + distance_surcharge
    ...
    def rate_service(self, new_rating: float) -> float:
        ...
        self.average_rating = round(total_score / self.count_of_ratings, 2)
